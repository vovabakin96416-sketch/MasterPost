 ...
 * Слова «да»/«нет» мапятся на общий пул «оракул» (см. triggerStage.ts).
 *
 * campaignStart здесь НЕТ намеренно: старт плана — рантайм-состояние, его пишет только
 * `ensureCampaignStart` при первом запуске автопостинга. Пока он лежал в сиде со
 * значением `null`, каждый `npm run seed` затирал уже зафиксированный старт.
 *
