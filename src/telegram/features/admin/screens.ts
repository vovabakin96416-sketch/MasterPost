...
/**
 * Экран «🤖 Мой бот» (Шаг 14b-bis-1, запуск — 14b-bis-2) — свой бот клиента.
 *
 * ⚠️ ЧЕСТНОСТЬ ЭКРАНА: с 14b-bis-2 бот владельца реально поднимается и ведёт
 * УПРАВЛЕНИЕ (меню в личке), но публикация в канал всё ещё идёт через общего
 * бота (маршрутизация — 14b-bis-3). Экран говорит это прямо, иначе владелец
 * будет ждать от своего бота постов, которых пока не будет.
 */
export async function renderBotAccount(deps: AdminDeps): Promise<Screen> {
  ...
  if (account === null) {
    lines.push(
      ...
      "",
      "ℹ️ Сразу после подключения твой бот запустится и сможет вести управление (меню). Публиковать в канал он начнёт следующим обновлением.",
    );
  ...
  lines.push(
    ...
    "",
    "ℹ️ Управление через твоего бота уже работает. Публикацию в канал пока ведёт общий бот — переключение следующим обновлением.",
  );
