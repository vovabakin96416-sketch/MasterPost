...
/**
 * Экран «🤖 Мой бот» (14b-bis-1 хранение, -2 запуск, -3 маршрутизация).
 *
 * ⚠️ ЧЕСТНОСТЬ ЭКРАНА: с 14b-bis-3 бот владельца и ведёт управление, и публикует
 * в канал. Но публикация возможна, только если он там админ, — поэтому экран
 * говорит про это прямо и предупреждает, что иначе пост уйдёт общим ботом
 * (страховка маршрутизации: пост важнее того, чьим именем подписан).
 */
export async function renderBotAccount(deps: AdminDeps): Promise<Screen> {
  ...
  if (account === null) {
    lines.push(
      ...
      "",
      "ℹ️ Сразу после подключения твой бот запустится, возьмёт на себя управление и публикацию. Если он не будет админом в канале, пост уйдёт общим ботом — не пропадёт.",
    );
  ...
  lines.push(
    ...
    account.lastError === null
      ? "Состояние: бот запущен ✅"
      : `Состояние: последняя ошибка — ${account.lastError} ⚠️`,
    "",
    `Напиши @${account.username} /start — без этого он не сможет присылать тебе сообщения.`,
    "Проверь, что этот бот добавлен админом в твой канал с правом «Публиковать сообщения».",
    "",
    "ℹ️ Твои посты и сообщения идут через него. Если он не сможет отправить (не админ в канале, не нажат /start) — сообщение уйдёт общим ботом, чтобы ничего не пропало.",
  );
