...
## 🔜 Сейчас
**РЕДИЗАЙН МЕНЮ (вар. B) — ПОДШАГ 1 ГОТОВ** (typecheck 0, lint 0, vitest **407/407**, build ок,
миграций НЕТ, env НЕТ):
СЛИЯНИЕ «Календарь» + «Контент-план» → единый раздел «📅 План». Убирает главную путаницу IA
(дубль на одних данных `Post`, оба вели в один редактор `pp`). Проект редизайна — в `.claude/plans/`
(диагностика + варианты A/B + карта «старое→новое»); интерактивный макет вар. B — Artifact.
Решение владельца (2026-07-13): берём вариант B, реализуем в 3 подшага.
- ГЛАВНОЕ МЕНЮ: «🗂 Контент-план»(`plan`) + «📅 Календарь»(`cal`) → ОДНА «📅 План»(`cal`); 13→12 кнопок.
  Callback-коды `cal`/`plan` не тронуты (совместимость).
- ВХОД `renderCalendar`(`cal`) — текущая неделя, заголовок «📅 План», кнопка «🗂 Весь план (4 нед.)»→`plan`.
- ПОД-ЭКРАН `renderPlan`(`plan`) — «🗂 Весь план»; «◀ Назад» ведёт к `cal` (ровная иерархия).
- Указатели: напоминание конца контента (`analyticsService`) → «📅 План → 🗂 Весь план».
- ⏭ Дальше — ПОДШАГ 2: hub «💬 Комментарии» (переим. «⚙️ Настройки» + перенос «💬 Триггеры» +
  «📊 Статус»→«📋 Сводка»). Затем ПОДШАГ 3: hub «📈 Рост» (Аналитика→«📨 Отчёт по просмотрам»,
  подъём Экспериментов) + финальный главный экран вар. B (быстрые действия + бейдж «Одобрение (N)»)
  + slim «⚙️ Настройки».
- ⏳ За пользователем — проверка в TG: /menu → «📅 План» → «🗂 Весь план (4 нед.)» → неделя → пост.
