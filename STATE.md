...
## 🔜 Сейчас
Аудит надёжности для автономной работы завершён, 4 фикса внедрены: `protect: true` в
cron-джобах (дубли постов при наложении тиков), `@grammyjs/auto-retry` (429/сеть),
`saveProgress` после каждого времени (рестарт посреди цикла), онбординг только от
владельца (`my_chat_member.from.id === adminId`). Бэклог аудита (🟢) — в записи
ARCHIVE-PROGRESS: vitest@4, MarkdownV2, runner, TTL PendingPost.
...
Тесты сейчас: **vitest 177/177**, tsc 0, eslint 0.
