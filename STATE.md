...
- ⏳ ЗА ВЛАДЕЛЬЦЕМ: (а) прогнать `npx tsx scripts/verify14b1.tmp.ts` (локальная проверка на БД:
  гейт/скоуп/штампы/не-перехват; файл не в git, данные чистит сам; запуск скрипта ботом заблокировал
  классификатор разрешений); (б) в TG: /menu работает как раньше, «📡 Каналы» → видна кнопка
  «➕ Пригласить владельца»; (в) для триала: пригласить тестера по id → тестер /menu → добавляет
