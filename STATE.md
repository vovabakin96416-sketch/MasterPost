...
## 🔜 Сейчас
Шаг 10b **введён в прод и проверен** — `ANTHROPIC_API_KEY` добавлен на Railway (сервис бота),
миграция `pexelsQuery` применилась, бот поднялся, кнопка «🤖 AI-пост» вернула превью голосом
канала (полнолуние + фото + CTA); «🔄 Другое фото» и «✍️ Изменить текст» работают. MTProto-
сессия — владелец подтвердил, сделана. Детали ввода в прод — в `docs/ARCHIVE-PROGRESS.md`.

**Следующий подшаг — 10c** (доводка Шага 10): автопостинг сам подхватывает AI-генерацию,
когда на слот нет готового поста (тумблер «🤖 AI когда план пуст»; одобрение — общим тумблером
«📋 Одобрение»), + кнопка «👀 Предпросмотр» на превью (показать пост как в канале, с кнопками).
После 10c закрываем Шаг 10 и переходим к Шагу 11 (Engagement Engine: AI-ответы в комментах).
Отложено в Шаге 10: Media Engine (AI-картинки) + роутинг моделей; ввод темы в мастер AI-поста.
