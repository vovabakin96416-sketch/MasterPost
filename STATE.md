...
## 🔜 Сейчас
UX-пакет админ-меню готов: главное меню сгруппировано 2×5 (контент → публикация →
комменты → канал → сводки), «➕ Новый пост» на главной, свежие тексты (без «запусти сид»
и устаревшей подсказки 8b/8c), единый термин «🎯 Канал публикации», `pluralRu` вместо
трёх копий склонений. Callback-коды не менялись — старые кнопки работают. Бэклог
аудита (🟢) — в записях ARCHIVE-PROGRESS: vitest@4, MarkdownV2, runner, TTL PendingPost,
дедупликация экранов триггеров/пулов.
...
Тесты сейчас: **vitest 181/181**, tsc 0, eslint 0.
